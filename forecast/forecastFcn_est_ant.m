...
%                forecasts, counterfactuals, shock decompositions, and smoothed
%                shock estimates, both unconditional and conditional on Dick
%                Peach's forecast.
%
...
for a = 1:nDraws
    ...
    % state selector
    A = mapStates(mspec,nstate,nplotstates,0);
    params = priotheta(a,:)';
    ...
    if zerobound
        [TTT,RRR,zend] = getmodel(TTTsim,RRRsim,zendsim,a,nstate+nant+1,...
            nshocks+nant);
    else
    ...
    if zerobound
        TTT_510 = zeros(nstate-(nant+1),nstate-(nant+1));
        iSplit = getState(mspec,0,'n_end+n_exo');
        TTT_510(1:iSplit,1:iSplit) = TTT(1:iSplit,1:iSplit);
        TTT_510(iSplit+1:end,1:iSplit) = TTT(iSplit+(nant+1)+1:end,1:iSplit);
        TTT_510(1:iSplit,iSplit+1:end) = TTT(1:iSplit,iSplit+(nant+1)+1:end);
        TTT_510(iSplit+1:end,iSplit+1:end) = TTT(iSplit+(nant+1)+1:end,...
            iSplit+(nant+1)+1:end);
        
        RRR_510 = zeros(nstate-(nant+1),nshocks);
        RRR_510(1:iSplit,1:end) = RRR(1:iSplit,1:nshocks);
        RRR_510(iSplit+1:end,1:end) = RRR(iSplit+(nant+1)+1:end,1:nshocks);
    else
        TTT_510 = TTT;
        RRR_510 = RRR;
    end
    ...
    nstate = size(TTT_510,1);
    [ZZ,DD,DDcointadd,QQ,EE,MM,retcode] = getmeasur(mspec,TTT_510,RRR_510,...
        valid_para,params,nvar,...
        nlags,npara,coint,cointadd);
    ...
    HH = EE+MM*QQ*MM';
    VV = QQ*MM';
    VVall = [[RRR_510*QQ*RRR_510',RRR_510*VV];[VV'*RRR_510',HH]];
    lead = 1;
    
    % Define the initial mean and variance for the state vector
    % (deals with nonstationarity on model by model basis)
    [A0,P0] = lyap_nonstationary(mspec,params,TTT_510,RRR_510,QQ);
    
    % Kalman filtering over the presample
    if ~isempty(YY0)
        [pyt0,zend,pend,pred0,vpred0] = kalcvf2NaN(YY0(:,1:nvar-nant_rm)',1,zeros(nstate,1),TTT_510,...
            DD,ZZ,VVall,A0,P0);
    else
        zend = A0;
        pend = P0;
    ...
    if zerobound == 1
        ...
        [L,zend,pend,pred,vpred] = ...
            kalcvf2NaN(YY(1:end-antlags-1,1:nvar-nant_rm)',lead,...
            zeros(nstate,1),TTT_510,DD,ZZ,VVall,zend,pend);
        ...
        [ZZ,DD,DDcointadd,QQ,EE,MM,retcode] = ...
            getmeasur(mspec,TTT,RRR,valid_a,params,nvar,nlags,npara,...
            coint,cointadd,nant);
    ...
    ind_r = find(strcmp(varnames,'Interest Rate'));
    ind_r_sh = find(strcmp(names_shocks,'Money'));
    ...
    if peachflag
        ...
        if zerobound
            ...
            [yypred] = ...
                ...
                zerobound,0,nant,sflag,A,nplotstates,mspec);
            ...
            [yypred] = ...
                getForecast_s(qahead_extra,nvar,nshocks,zeros(size(TTT,1),1),TTT,...
                RRR,DD,ZZ,QQ,zend_peach,ind_r,ind_r_sh,zerobound,1,...
                nant,sflag,A,nplotstates,mspec);
            yypred_nocoint = [peachdataimplied; ...
                yypred(:,1:nvar)];
    ...
    if zerobound
        ...
        [yypred] = ...
            ...
            DD,ZZ,QQ,zend_uncond,ind_r,ind_r_sh,zerobound,0,...
            nant,sflag,A,nplotstates,mspec);
        fcast.uncond(a,:) = yypred(:)';
        
        % Calculate bounded interest rate forecast
        [yypred] = ...
            getForecast_s(qahead,nvar,nshocks,zeros(size(TTT,1),1),TTT,RRR,DD,...
            ZZ,QQ,zend_uncond,ind_r,ind_r_sh,zerobound,1,nant,...
            sflag,A,nplotstates,mspec);
